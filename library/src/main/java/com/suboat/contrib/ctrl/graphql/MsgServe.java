...
import com.suboat.contrib.rpc.msg.MessageService;
import lombok.Data;
...
@Data
public class MsgServe {

	private static final Logger log = LoggerFactory.getLogger(MsgServe.class);
	...
	//
	public MsgServe(RpcConnConfig cfg) {
		config = cfg;
		syncConfig();
	}

	//
	public MsgServe(String addr) {
	...
	public MessageService.Client getClient() {
		...
		try {
			log.info("rpc-msg conn {}:{} open", config.host, config.port);
			if (config.framed) {
	...
	public void close() {
		...
		log.info("rpc-msg conn {}:{} close", config.host, config.port);
		transport.close();
		transport = null;
		client = null;
	}
